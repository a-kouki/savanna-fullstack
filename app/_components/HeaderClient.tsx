...
export function HeaderClient({heroBanner}: HeaderClientProps) {
  ...
  return (
    <>
      ...
      >
        ...
        {heroBanner ? (
          <BannerMedia banner={heroBanner} priority className="object-cover object-top" />
        ) : (
          <div className="absolute right-0 top-0 h-full w-[55%] bg-zinc-700/40" />
        )}

        {/* Conteúdo hero */}
        <div className="relative z-10 flex flex-col justify-end min-h-screen pb-20 px-6 md:px-pxGloabl max-w-widthGlobal mx-auto">
          ...
          <p className="font-abeezee text-white/70 mt-4 text-sm sm:text-base max-w-[400px]">
            As melhores camisetas do futebol, com qualidade e estilo.
          </p>
          <div className="flex gap-4 mt-8">
            <a
              href="#camisetas"
              className="bg-secondary text-black font-bebas tracking-widest text-lg px-8 py-3 hover:bg-yellow-300 transition-colors duration-200"
            >
              Ver Camisetas
            </a>
            <a
              href="#contato"
              className="border border-white/40 text-white font-abeezee text-sm px-8 py-3 hover:border-secondary hover:text-secondary transition-colors duration-200"
            >
              Contato
            </a>
          </div>
